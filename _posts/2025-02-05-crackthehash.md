...
tags: [linux, name-that-hash, hashcat, john-the-ripper, rainbow-tables, hashes.com]
render_with_liquid: false
media_subpath: /images/crackthehash/
---

Para este desafio, foi utilizado o identificador de hash `Name-That-Hash` e as ferramentas `John the Ripper` e `Hashcat` para a quebra dos hashes.

Antes de começar a quebra dos hashes precisamos identificar cada tipo, então fiz toda a enumeração de hash primeiro.
...
```console
┌──(c3n0r4㉿kali)-[~/tryhackme/room/crackthehash
└─$ haiti $2y$12$Dwt1BZj6pcyc3Dy1FWZ5ieeUznr71EeNkJkUlypTsgbX1H68wsRom                   
Unknown hash type
```

O `Name-That-Hash` não conseguiu identificar o tipo de hash, então analisei a dica fornecida para essa questão na sala.

> Dica: Pesquise na [página de exemplos do hashcat](https://hashcat.net/wiki/doku.php?id?example_hashes) por $2y$. Este tipo de hash pode levar muito tempo para ser quebrado, então filtre a wordlist rockyou para quatro palavras, ou usar uma máscara para quatro caracteres minúsculo. 
 {: .prompt-tip }

Com base na dica, identifiquei que o tipo de hash é o `bcrypt`
...
Session completed. 
```
`John the Ripper` executou o ataque contra o hash fornecido usando o formato `raw-md5`, mas não conseguiu encontrar a senha correspondente. Utilizando a dica fornecida para essa questão na sala.

> Dica: md4
 {: .prompt-tip }

Executei um novo ataque agora utilizando o formato `raw-md4`
...
```
> Realizando uma pesquisa sobre a utilização do hash `bigcrypt` via `John the Ripper`, obtive a seguinte resposta:
O formato `bigcrypt` não está presente na versão atual do `John the Ripper Jumbo` porque:
- `Bigcrypt` é um formato antigo e raramente utilizado, originalmente criado para hashes DES baseados em arquivos grandes.
- Nas versões modernas do `John Jumbo`, esse formato foi removido, renomeado ou incorporado a outro formato.
- Além disso, a maioria dos hashes atuais utiliza formatos mais modernos, como `bcrypt`, `sha512crypt`, entre outros.
Portanto, para trabalhar com hashes atuais, é recomendado utilizar os formatos suportados e mantidos oficialmente pelo John the Ripper Jumbo.
{: .prompt-info }

Seguindo a lógica do hash `$6$aReallyHardSalt$6WKUTqzq.UQQmrm0p/T7MPpMbGNnzXPMAXi4bJMl9be.cfi3/qxIf.hsGpS41BqMhSrHVXgMpdjS6xeKZAs02` apresentado na tarefa 1, realizei uma pesquisa sobre hashes que começam com `$6$`, e identifiquei que se trata do formato `SHA512-Crypt`.
...
Outra opção muito mais agil para a resolução desta sala é o uso da plataforma [hashes.com](https://hashes.com/en/decrypt/hash), que trabalha com o conceito de **rainbow tables**.
